...
def stem(word):
	...
	for _ in word:
		...
		if (word_reminder in StemmingRules) and (stem in StemmingRules[word_reminder]):
			'Return stemmed word'
...
try: StemmingRules = pickle.load(open('rules/StemmingRules-MinFreq-'+str(MIN_RULE_FREQ)+'.pickle', 'rb'))
